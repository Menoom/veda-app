            ...
            <div key={veda.name} className="relative group">
              ...
              <div className={`hidden md:flex items-center ${isEven ? 'flex-row' : 'flex-row-reverse'}`}>
                ...
                <div className={`w-1/2 ${isEven ? 'pr-8 text-right' : 'pl-8 text-left'}`}>
                  <div className="bg-glass rounded-2xl p-6 shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-105">
                    <h3 className="font-serif text-2xl font-bold text-primary mb-2">{veda.name}</h3>
                    <p className="text-earth-600 mb-4">{veda.description}</p>
                    <div className="text-sm text-earth-500 mb-4">{veda.verses}</div>
                    
                    {/* Progress Bar */}
                    <div className="mb-4">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-sm text-earth-600">Progress</span>
                        <span className="text-sm font-semibold text-primary">{veda.progress}%</span>
                      </div>
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div 
                          className={`h-2 rounded-full transition-all duration-300 ${getColorClasses(veda.color, 'bg')}`}
                          style={{ width: `${veda.progress}%` }}
                        ></div>
                      </div>
                    </div>
                    
                    <button className={`px-4 py-2 rounded-lg transition-all duration-200 ${getColorClasses(veda.color, 'text')} hover:bg-opacity-10 hover:bg-current`}>
                      Continue Reading
                    </button>
                  </div>
                </div>
                ...
                <div className="relative z-10">
                  <div className={`w-16 h-16 ${getColorClasses(veda.color, 'bg')} rounded-full flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform`}>
              ...
              <div className="md:hidden">
                <div className="flex items-start space-x-4">
                  <div className={`w-12 h-12 ${getColorClasses(veda.color, 'bg')} rounded-full flex items-center justify-center shadow-lg flex-shrink-0`}>
                    <Icon className="w-6 h-6 text-white" />
                  </div>
                  <div className="flex-1 bg-glass rounded-xl p-4 shadow-lg">
                    <h3 className="font-serif text-xl font-bold text-primary mb-2">{veda.name}</h3>
                    <p className="text-earth-600 mb-3">{veda.description}</p>
                    <div className="text-sm text-earth-500 mb-3">{veda.verses}</div>
                    ...
                    <div className="mb-3">
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-1.5">
                        <div 
                          className={`h-1.5 rounded-full transition-all duration-300 ${getColorClasses(veda.color, 'bg')}`}
                          style={{ width: `${veda.progress}%` }}
                        ></div>
                      </div>
                    </div>
                    
                    <button className={`px-3 py-1.5 rounded-lg text-sm transition-all duration-200 ${getColorClasses(veda.color, 'text')} hover:bg-opacity-10 hover:bg-current`}>
                      Continue Reading
                    </button>
                  </div>
                </div>
